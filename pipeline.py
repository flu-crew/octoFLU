...
subprocess.call([BLASTN,"-db",reference,"-query",baseName + ".clean","-num_alignments","1","-outfmt","6","-out",outDir + "/blast_output.txt"])
...
with open(outDir + "/H1.ids", 'w') as file:
	file.writelines(["%s\t" % item  for item in H1list])
with open(outDir + "/H3.ids", 'w') as file:
	file.writelines(["%s\t" % item  for item in H3list])
with open(outDir + "/N1.ids", 'w') as file:
	file.writelines(["%s\t" % item  for item in N1list])
with open(outDir + "/N2.ids", 'w') as file:
	file.writelines(["%s\t" % item  for item in N2list])
with open(outDir + "/PB2.ids", 'w') as file:
	file.writelines(["%s\t" % item  for item in PB2list])
with open(outDir + "/PB1.ids", 'w') as file:
	file.writelines(["%s\t" % item  for item in PB1list])
with open(outDir + "/PA.ids", 'w') as file:
	file.writelines(["%s\t" % item  for item in PAlist])
with open(outDir + "/NP.ids", 'w') as file:
	file.writelines(["%s\t" % item  for item in NPlist])
with open(outDir + "/M.ids", 'w') as file:
	file.writelines(["%s\t" % item  for item in Mlist])
with open(outDir + "/NS.ids", 'w') as file:
	file.writelines(["%s\t" % item  for item in NSlist])
